...
# This code is modified by X.Zhang from
# https://github.com/facebookresearch/pycls/blob/master/pycls/models/effnet.py
#
...
from pycls.losses.rbflogit import RBFLogits, LinearLogits
...
class EffHead(nn.Module):
    ...
    def __init__(self, w_in, w_out, nc):
        ...
        if cfg.MODEL.LOGITS_TYPE=="rbf_logit":
            self.fc = RBFLogits(w_out, nc, cfg.MODEL.RBF_SCALE, cfg.MODEL.RBF_SCALE)
        else:
            self.fc = LinearLogits(w_out, nc)

    def forward(self, x, label):
        x = self.conv_swish(self.conv_bn(self.conv(x)), label)
        x = self.avg_pool(x)
        x = x.view(x.size(0), -1)
...
class SE(nn.Module):
    ...
    def forward(self, x, label):
        x1 = self.avg_pool(x)
        x1 = self.conv1(x1)
        x1 = self.swish1(x1, label)
        return x * self.f_ex(x1)
...
class MBConv(nn.Module):
    ...
    def forward(self, x, label):
        f_x = x
        if self.exp:
            f_x = self.exp_swish(self.exp_bn(self.exp(f_x)), label)
        f_x = self.dwise_swish(self.dwise_bn(self.dwise(f_x)), label)
        f_x = self.se(f_x, label)
