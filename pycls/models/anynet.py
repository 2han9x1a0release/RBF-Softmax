...
# https://github.com/facebookresearch/pycls/blob/master/pycls/models/anynet.py
#
...
from pycls.losses.rbflogit import RBFLogits
...
class AnyHead(nn.Module):
    ...
    def __init__(self, w_in, nc):
        ...
        if cfg.MODEL.LOGITS_TYPE=="rbf_logit":
            self.fc = RBFLogits(w_in, nc, cfg.MODEL.RBF_SCALE, cfg.MODEL.RBF_SCALE)
        else:
            self.fc = LinearLogits(w_in, nc)
...
class ResBottleneckBlock(nn.Module):
    ...
    def forward(self, x, label):
        if self.proj_block:
            x = self.bn(self.proj(x)) + self.f(x, label)
        else:
            x = x + self.f(x, label)
        x = self.relu(x)
        return x
